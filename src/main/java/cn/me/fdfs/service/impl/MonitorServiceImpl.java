...
public class MonitorServiceImpl extends BaseService implements MonitorService {
	...
	public List<Line> getNetTrafficLines(String ip, String start, String end) {
		...
		List storages = query.setParameter("ip", ip).setParameter("starttime", starttime)
	...
	public List<Line> listStorageLines(String groupName) throws IOException, MyException {
		...
		for (Storage s : storages) {
			...
			for (int i = results.size() - 1; i >= 0; i--) {
				...
				logger.debug("created date={}, memory={}", ss.getCreated(), ss.getMem());
				long createTime;
				if (ss.getCreated() == null) {
					createTime = System.currentTimeMillis();
				} else {
					createTime = ss.getCreated().getTime();
				}
				String cpu = "0";
				if (ss.getCpu() != null && ss.getCpu().length() != 0) {
					cpu = ss.getCpu();
				}

				line.getData().add(new Object[] { createTime, ss.getMem() });
				line1.getData().add(new Object[] { createTime, Double.parseDouble(cpu) });
	...
	public List<Line> getListFileCountStorage(String ip, String startTime, String endTime) throws IOException,
		...
		List<StorageHour> storages = query.setString("ip", ip).setParameter("starttime", start)
				.setParameter("endtime", end).list();
